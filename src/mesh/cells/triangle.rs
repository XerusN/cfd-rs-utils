...
impl Cell2D for Triangle {
    ...
    fn new(edges_idx: &[EdgeIndex], neighbors: &[Neighbor2D]) -> Result<Self, Error> {
        ...
        if neighbors.len() != 3 {
            return Err(Error::WrongSliceLength { got: neighbors.len(), expected: 3 });
        }
        ...
        Ok(Triangle {
            ...
            neighbors: [neighbors[0].clone(), neighbors[1].clone(), neighbors[2].clone()],
        })
    }
    ...
    fn fast_edges_length(&self) -> Vec<f64> {
        vec![(self.nodes[1] - self.nodes[0]).magnitude(), (self.nodes[2] - self.nodes[1]).magnitude(), (self.nodes[0] - self.nodes[2]).magnitude()]
    }
