...
use super::{indices::{CellIndex, FaceIndex, ParentIndex, VertexIndex, BoundaryPatchIndex, HalfEdgeIndex}, Base2DMesh, Parent};
...
impl Face {
    ...
    pub fn new(vertices: [VertexIndex; 2], patches: (Patch, Patch), vertices_glob: &[Point2<f64>]) -> Self {
        ...
        Face { vertices, area, normal, patches }
    }
}
...
impl Cell {
    pub fn new(faces: Vec<FaceIndex>, vertices_glob: &[Point2<f64>], faces_glob: &[Face]) -> Self {
        ...
        for face in faces.iter().map(|f_id| &faces_glob[f_id.0]).collect::<Vec<&Face>>() {
            for i in 0..2 {
        ...
        let points = [vertices_glob[vertices[0].0], vertices_glob[vertices[1].0], vertices_glob[vertices[2].0]];
        let volume = triangle_area(&points);
        let centroid = triangle_centroid(&points);
        Cell { volume, centroid, faces, vertices }
    }
    ...
    pub fn faces<'a>(&self, faces_glob: &'a [Face]) -> Vec<&'a Face> {
        self.faces.iter().map(|f_id| &faces_glob[f_id.0]).collect::<Vec<&Face>>()
    }
    ...
    pub fn vertices<'a>(&self, vertices_glob: &'a [Point2<f64>]) -> Vec<&'a Point2<f64>> {
        self.vertices.iter().map(|f_id| &vertices_glob[f_id.0]).collect::<Vec<&Point2<f64>>>()
    }
}
    ...
    pub fn new_from_he(mesh: Base2DMesh) -> Self {
        ...
        for (he, &HalfEdgeIndex(twin)) in mesh.he_to_twin().iter().enumerate() {
            if he == twin {
                ...
                let patches = (parent_to_patch[mesh.he_to_parent()[he].0].clone(), parent_to_patch[mesh.he_to_parent()[twin].0].clone());
                faces.push(Face::new(mesh.vertices_from_he(HalfEdgeIndex(he)), patches, &vertices));
                i += 1;
            } else {
                he_to_face.push(he_to_face[he]);
            }
        }
            ...
            match patch {
                ...
                &Patch::Boundary(id) => {
                    let boundary = match mesh.parents()[parent].clone() {
                        ...
                        _ => panic!("Bad construction of parent_to_patch at index : {:?}", parent)
                    };
                    let faces_loc = mesh.he_from_parent(ParentIndex(parent)).iter().map(|he| he_to_face[he.0]).collect();
        ...
        Self{cells, faces, boundaries, vertices}
    }
